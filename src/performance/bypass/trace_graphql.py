...
with open("payload_raw.json") as f:
...
for group_id in groups:
    ...
    curl_cmd = [
        ...
        "-o", "-",  # Capture response body to stdout
        "-L",  # Follow redirects
        "-X", "POST",
        f"https://engage.cloud.microsoft/graphql?operationName=FeedGroupNestedClients&apiVnext={version}",
    ...
    # Separate timing footer from everything else
    if "=== CURL TIMING ===" in result.stdout:
        main_part, timing_text = result.stdout.split("=== CURL TIMING ===", 1)
    else:
        main_part = result.stdout
        timing_text = "[ERROR] Could not parse timing output."

    # Headers and body are both in main_part. The headers end at the *last* HTTP status line before JSON.
    http_split = re.split(r'\r?\n\r?\n', main_part, maxsplit=1)
    if len(http_split) == 2:
        header_text, json_body = http_split
    else:
        header_text = main_part
        json_body = ''
